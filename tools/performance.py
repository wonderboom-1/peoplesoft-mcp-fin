...
def register_tools(mcp):
        ...
        sql = f"""
            ...
            FROM (
                SELECT A.ACCOUNT, A.CURRENCY_CD,
                       SUM(A.MONETARY_AMOUNT) AS ACTUAL_AMT
                FROM PS_LEDGER A
            ...
            FULL OUTER JOIN (
                SELECT B.ACCOUNT, B.CURRENCY_CD,
                       SUM(B.MONETARY_AMOUNT) AS BUDGET_AMT
                FROM PS_LEDGER B
        ...
        sql = """
            ...
            FROM (
                ...
                WHERE B.BUSINESS_UNIT = :1 AND B.FISCAL_YEAR = :2
                  AND B.LEDGER = :3
                GROUP BY B.ACCOUNT, B.CURRENCY_CD
                HAVING SUM(B.MONETARY_AMOUNT) <> 0
            ) BUD
            LEFT JOIN (
                SELECT A.ACCOUNT, A.CURRENCY_CD,
                       SUM(A.MONETARY_AMOUNT) AS ACTUAL_AMT
    ...
    async def get_commitment_control_budget(
        ...
        sql = f"""
            SELECT
                ...
                K.CURRENCY_CD,
                K.ACCOUNTING_PERIOD,
                SUM(CASE WHEN K.LEDGER = 'BUDGETS' THEN K.MONETARY_AMOUNT ELSE 0 END) AS BUDGET_AMT,
                SUM(CASE WHEN K.LEDGER = 'PRE_ENCMBR' THEN K.MONETARY_AMOUNT ELSE 0 END) AS PRE_ENCUMBRANCE,
                SUM(CASE WHEN K.LEDGER = 'ENCUMBRNCE' THEN K.MONETARY_AMOUNT ELSE 0 END) AS ENCUMBRANCE,
                SUM(CASE WHEN K.LEDGER = 'ACTUALS' THEN K.MONETARY_AMOUNT ELSE 0 END) AS EXPENSE
            FROM PS_LEDGER_KK K
            WHERE K.BUSINESS_UNIT = :1
              AND K.LEDGER_GROUP = :2
              AND K.FISCAL_YEAR = :3
              {acct_cond}
        ...
        sql = f"""
            SELECT
                ...
                SUM(CASE WHEN K.LEDGER IN ('ACTUALS','ENCUMBRNCE','PRE_ENCMBR')
                         THEN K.MONETARY_AMOUNT ELSE 0 END) AS CONSUMED_AMT,
                SUM(CASE WHEN K.LEDGER = 'BUDGETS' THEN K.MONETARY_AMOUNT ELSE 0 END)
                  - SUM(CASE WHEN K.LEDGER IN ('ACTUALS','ENCUMBRNCE','PRE_ENCMBR')
                             THEN K.MONETARY_AMOUNT ELSE 0 END) AS AVAILABLE_BALANCE,
                CASE WHEN SUM(CASE WHEN K.LEDGER = 'BUDGETS' THEN K.MONETARY_AMOUNT ELSE 0 END) <> 0
                     THEN ROUND(
                         SUM(CASE WHEN K.LEDGER IN ('ACTUALS','ENCUMBRNCE','PRE_ENCMBR')
                                  THEN K.MONETARY_AMOUNT ELSE 0 END)
                         / SUM(CASE WHEN K.LEDGER = 'BUDGETS' THEN K.MONETARY_AMOUNT ELSE 0 END) * 100, 2)
                     ELSE NULL END AS PCT_CONSUMED,
            ...
            HAVING SUM(CASE WHEN K.LEDGER = 'BUDGETS' THEN K.MONETARY_AMOUNT ELSE 0 END) <> 0
            ORDER BY K.ACCOUNT
            FETCH FIRST :{idx} ROWS ONLY
        """
        result = await execute_query(sql, params)
    ...
    async def get_period_close_status(
        ...
        Provides a summary count of open vs closed ledger groups per period.
        Supplements list_open_periods with an aggregate view.
        ...
        period_cond = ""
        if accounting_period is not None:
            period_cond = " AND O.ACCOUNTING_PERIOD = :3 "
        ...
        sql = f"""
            SELECT
                O.ACCOUNTING_PERIOD,
                O.OPEN_STATUS,
                COUNT(DISTINCT O.LEDGER_GROUP) AS LEDGER_GROUP_COUNT,
                LISTAGG(O.LEDGER_GROUP, ', ') WITHIN GROUP (ORDER BY O.LEDGER_GROUP) AS LEDGER_GROUPS
            FROM PS_OPEN_PERIOD O
            WHERE O.BUSINESS_UNIT = :1
              AND O.FISCAL_YEAR = :2
              {period_cond}
            GROUP BY O.ACCOUNTING_PERIOD, O.OPEN_STATUS
            ORDER BY O.ACCOUNTING_PERIOD, O.OPEN_STATUS
        """
        result = await execute_query(sql, params)
        if "error" in result:
            return result
    ...
    async def get_journal_posting_summary(
        ...
        sql = f"""
            SELECT
                ...
                SUM(H.JRNL_LINE_COUNT) AS TOTAL_LINES,

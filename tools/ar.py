...
def register_tools(mcp):
    ...
    async def get_customer(setid: str, cust_id: str) -> dict:
        """
        Get customer master (PS_CUSTOMER) current effective row.
        ...
        sql = """
            ...
            AND C.EFFDT = (
                SELECT MAX(C2.EFFDT) FROM PS_CUSTOMER C2
                WHERE C2.SETID = C.SETID AND C2.CUST_ID = C.CUST_ID
    ...
    async def search_customers(
        ...
        cond = [
            ...
            """C.EFFDT = (
                SELECT MAX(C2.EFFDT) FROM PS_CUSTOMER C2
                WHERE C2.SETID = C.SETID AND C2.CUST_ID = C.CUST_ID AND C2.EFFDT <= SYSDATE
            )""",
        ]
        n = 2
        if name_pattern:

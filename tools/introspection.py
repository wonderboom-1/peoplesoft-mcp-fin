...
def register_tools(mcp):
    ...
    async def describe_table(table_name: str) -> dict:
        ...
        if not result.get("results"):
            # Fallback: PeopleTools system tables (PSROLEUSER, PSOPERATION, etc.)
            # aren't in PSRECFIELD — try Oracle's ALL_TAB_COLUMNS directly.
            orig_name = table_name.upper().strip()
            sys_candidates = [orig_name]
            if not orig_name.startswith("PS"):
                sys_candidates.append("PS" + orig_name)
            if orig_name.startswith("PS_"):
                sys_candidates.append(orig_name[3:])
                sys_candidates.append("PS" + orig_name[3:])

            for sys_name in dict.fromkeys(sys_candidates):
                sys_result = await execute_query(
                    "SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH, NULLABLE, COLUMN_ID "
                    "FROM ALL_TAB_COLUMNS WHERE TABLE_NAME = :1 "
                    "ORDER BY COLUMN_ID",
                    [sys_name],
                )
                if "error" not in sys_result and sys_result.get("results"):
                    cols = []
                    for r in sys_result["results"]:
                        cols.append({
                    ...
                    return {
                        "table_name": sys_name,
                        "source": "ALL_TAB_COLUMNS (PeopleTools system table)",
            ...
            msg = (
                ...
                "or ALL_TAB_COLUMNS. Try list_tables(pattern='...') to search."
            )
            if similar:

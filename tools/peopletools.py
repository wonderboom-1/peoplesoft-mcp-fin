...
async def get_peoplecode(
    ...
    if include_code:
        sql = f"""
            SELECT
                ...
                PROGSEQ,
                DBMS_LOB.SUBSTR(PCTEXT, 4000, 1)     AS PCTEXT1,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 4000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 4001)
                     ELSE NULL END                    AS PCTEXT2,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 8000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 8001)
                     ELSE NULL END                    AS PCTEXT3,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 12000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 12001)
                     ELSE NULL END                    AS PCTEXT4,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 16000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 16001)
                     ELSE NULL END                    AS PCTEXT5,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 20000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 20001)
                     ELSE NULL END                    AS PCTEXT6,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 24000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 24001)
                     ELSE NULL END                    AS PCTEXT7,
                CASE WHEN DBMS_LOB.GETLENGTH(PCTEXT) > 28000
                     THEN DBMS_LOB.SUBSTR(PCTEXT, 4000, 28001)
                     ELSE NULL END                    AS PCTEXT8,
    ...
    if "error" not in result and include_code:
        chunk_cols = ("PCTEXT1", "PCTEXT2", "PCTEXT3", "PCTEXT4",
                      "PCTEXT5", "PCTEXT6", "PCTEXT7", "PCTEXT8")
        for row in result.get("results", []):
            pieces: list[str] = []
            total = 0
            for col in chunk_cols:
                txt = row.pop(col, None)
                if not txt:
                    break
                if hasattr(txt, "read") and callable(txt.read):
                    txt = txt.read()
                if not isinstance(txt, str) or not txt:
                    break
                remaining = max_code_length - total
                if remaining <= 0:
                    break
                pieces.append(txt[:remaining] if len(txt) > remaining else txt)
                total += len(txt)
            row["PEOPLECODE"] = "".join(pieces)
...
async def get_permission_list_details(permission_list: str) -> dict:
    ...
    # Get permission list header from PSCLASSDEFN
    # Actual columns: CLASSID, VERSION, CLASSDEFNDESC, TIMEOUTMINUTES, DEFAULTBPM,
    #   STARTAPPSERVER, ALLOWPSWDEMAIL, LASTUPDDTTM, LASTUPDOPRID
    header_sql = """
        SELECT 
            C.CLASSID AS PERMISSION_LIST,
            C.CLASSDEFNDESC AS DESCRIPTION,
            C.LASTUPDDTTM,
            C.LASTUPDOPRID
        FROM PSCLASSDEFN C
        WHERE C.CLASSID = :1
    ...
    # Get menu/component access from PSAUTHITEM
    # Actual columns: CLASSID, MENUNAME, BARNAME, BARITEMNAME, PNLITEMNAME,
    #   DISPLAYONLY, AUTHORIZEDACTIONS
    component_sql = """
        SELECT 
            A.MENUNAME,
            A.BARNAME,
            A.BARITEMNAME,
            A.PNLITEMNAME AS COMPONENT,
            A.DISPLAYONLY,
            A.AUTHORIZEDACTIONS
        FROM PSAUTHITEM A
        WHERE A.CLASSID = :1
...
async def get_process_definition(process_name: str | None = None, process_type: str | None = None) -> dict:
    """
...
async def get_integration_broker_services(service_name: str | None = None) -> dict:
    """
    Get Integration Broker service operations and their configuration.
    Queries PSOPERATION for operation definitions. Use service_name to
    filter by operation name pattern.

    Args:
        service_name: Optional operation/service name pattern filter

    Returns:
        Operation definitions with type, messages, and handler info
    """
    params = []
    where_clause = ""

    if service_name:
        where_clause = "WHERE S.IB_OPERATIONNAME LIKE :1"
